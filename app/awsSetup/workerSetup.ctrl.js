...
.controller('WorkerSetupCtrl', ['$log', '$scope', 'localStorageService', '$http', 'awsService', '$q', function($log, $scope, localStorageService, $http, awsService, $q) {
    ...
    $http.get('amiList.json').then(function(response) {
        ...
        response.data.amis.forEach(function(item) {
            var ami = {name: item.ami, version: item.blenderVersion, info: item.info}
    ...
    $scope.getSubnetIdForAvailabilityZone = function(availabilityZone) {
        return $scope.vpcData.subnets[availabilityZone];
    };
    ...
    $scope.$on('aws-spotprice-update', function(event, data) {
        ...
        if (data.NextToken) {
            awsService.getSpotPrices($scope.instances, data.NextToken);
        }
    });
    ...
    $scope.updateTypes = function() {
        $q.all([$http.get('instances.json'), awsService.getAvailabilityZones()])
            .then(function(results) {
                ...
                awsService.getSpotPrices($scope.instances);
            });
    };

    $scope.vpcData = {};

    $scope.updateSubnets = function() {
        awsService.getSubnets('vpc-d52f5eb2', function (data) {
			$scope.vpcData.subnets = {};

			data.Subnets.forEach(function(subnet) {
                $scope.vpcData.subnets[subnet.AvailabilityZone] = subnet.SubnetId;
			});
        });
    };
    ...
    $scope.generateScript = function() {
        ...
        script += '#sudo apt-get update\n' +
            ...
            'export NODE_INSTANCE_TYPE=$(curl http://169.254.169.254/latest/meta-data/instance-type)\n' +
            'mkdir -p "$B"\n' +
            'cd "$B"\n' +
    ...
    $scope.requestInstances = function() {
        ...
        $log.info('subnetId: '+ $scope.vpcData.subnetId);

        if ($scope.instanceType === 'spot') {
        	awsService.requestSpot($scope.amiSelect, $scope.sshKey, 'sg-131cd269', $scope.generateScript(), $scope.instance.size, snapshots, $scope.spotPrice, $scope.numInstances, 'one-time', $scope.queue.workQueue, $scope.s3.frameDestination.split('//').pop(), $scope.showStatus, $scope.vpcData.subnetId);
        } else {
        	//requestOndemand: function(ami, keyPair, securityGroup, userData, instanceType, count)
        	awsService.requestOndemand($scope.amiSelect, $scope.sshKey, 'brenda-web', $scope.generateScript(), $scope.instance.size, snapshots, $scope.numInstances, $scope.queue.workQueue, $scope.s3.frameDestination.split('//').pop(), $scope.showStatus, $scope.vpcData.subnetId);
        }
    };
